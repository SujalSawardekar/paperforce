      ...
      {wide1 && (
        <Link href={`/blog/${wide1.slug}`} className="group relative w-full h-[500px] lg:h-[600px] rounded-3xl overflow-hidden flex items-end p-8 lg:p-16">
          ...
          <div className="relative z-10 max-w-3xl">
            <div className="flex flex-nowrap items-center gap-2 xl:gap-4 mb-6 text-[10px] xl:text-sm font-semibold uppercase tracking-wider text-blue-200 whitespace-nowrap overflow-hidden">
              <span className="bg-blue-600 text-white px-2 xl:px-3 py-1 rounded-full shrink-0">{wide1.category}</span>
              <span className="w-1 h-1 xl:w-1.5 xl:h-1.5 rounded-full bg-blue-400 shrink-0" />
              <span className="shrink-0">{wide1.publishedDate}</span>
            </div>
        ...
        <Link href={`/blog/${editorial1.slug}`} className="group grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-0 items-center bg-slate-50 rounded-3xl overflow-hidden border border-slate-100">
          <div className="lg:col-span-5 p-8 lg:p-16 flex flex-col h-full justify-center">
             <div className="flex flex-nowrap items-center gap-2 xl:gap-4 mb-6 text-[10px] xl:text-sm font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap overflow-hidden">
              <span className="text-blue-600 shrink-0">{editorial1.category}</span>
              <span className="w-1 h-1 xl:w-1.5 xl:h-1.5 rounded-full bg-slate-300 shrink-0" />
              <span className="flex items-center gap-1 xl:gap-1.5 shrink-0"><Clock size={14} className="shrink-0 xl:w-4 xl:h-4" /> {editorial1.readTime}</span>
            </div>
